...
execute as @e[family=mtc_body,tag=mtc_parent,scores={mtc_calc1=0},c=1] if score @s mtc_not matches !0 run playsound mtc.notch @p[r=30] ~~~ 100

execute as @e[family=mtc_body,tag=mtc_parent,scores={mtc_calc1=0},c=1] at @s if score @s mtc_not matches !0 run scoreboard players set @s mtc_not 0
