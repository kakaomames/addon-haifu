...
# 全先頭車両が被連結車探索
execute as @e[family=mtc_body,r=30,c=1] if entity @s[scores={mtc_calc1=0}] run tag @s add mtc_automaster
...
execute as @e[family=mtc_body,r=30,c=1] if entity @s[scores={mtc_calc1=0}] run tag @e[family=mtc_body,tag=mtc_parent] add mtc_connsilent
